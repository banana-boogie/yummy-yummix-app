...
Domain playbook for the YummyYummix AI system — gateway, tool system, RAG, orchestrators, and AI-powered features.
...
**Primary reference:** `docs/architecture/CLAUDE-AI-ARCHITECTURE.md` — comprehensive 900-line architecture document. Read this for deep context.
...
2. Implement the same interface as `openai.ts`: `callProvider()`, `callProviderStream()`, `callProviderEmbedding()`
...
3. **Implement** the execute function — Either inline or as a separate file for complex tools
...
|-----------|--------|--------|
| Semantic (vector similarity) | 55% | pgvector cosine distance |
| Lexical (keyword matching) | 25% | Name, tag, keyword matches |
| Metadata (time, difficulty) | 10% | Filter constraints |
| Personalization (cuisine prefs) | 10% | User preferences |

### Key Thresholds
- Include threshold: 0.35
- Fallback threshold: 0.42
- Min results before fallback: 2
...
1. **Phase 1:** Send `recipe_partial` event with the AI-generated recipe immediately
...
Before calling the LLM, use heuristics to optimize:

- **Recipe intent:** `recipe-intent.ts` detects when user likely wants a recipe (forces `toolChoice: "required"`)
- **Modification detection:** `modification.ts` uses regex (`detectModificationHeuristic`) to detect recipe modification requests — bypasses LLM entirely
- **Meal context:** `meal-context.ts` extracts meal type (breakfast, lunch, dinner) for context
- **Template suggestions:** `suggestions.ts` uses templates instead of AI-generated suggestions — cheaper and faster
...
### Allergen Pre-Check
`allergen-filter.ts` checks generated recipes against user's known allergies before serving.

### Food Safety
`food-safety.ts` validates cooking parameters against USDA rules (stored in `food_safety_rules` table). Categories: poultry, ground_meat, red_meat, seafood, eggs.
...
Safety features: input sanitization (control chars, 2000 char limit), list sanitization (max 20 items), graceful degradation (continues with defaults if profile fetch fails).
...
- **RecipeCardSchema** — Published recipe cards (recipeId, name, imageUrl, totalTime, difficulty, portions)
- **GeneratedRecipeSchema** — AI-generated recipes with Thermomix parameters
- **IrmixyResponseSchema** — Unified AI response (message, recipes, customRecipe, suggestions, actions, safetyFlags)
