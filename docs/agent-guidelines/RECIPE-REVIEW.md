...
11. **Step text vs ingredient list (text-level coherence).** Each ingredient *named in step instruction text* must appear in `recipe_ingredients` for the same recipe. Two sub-cases, both fixable via `step_text_overrides`:
  ...
  This is the *text* check; the *link-table* check (`recipe_step_ingredients` rows misaligned with the step) is #12. Don't conflate them — text issues route to `step_text_overrides`; mechanical link-table issues route to `step_ingredient_adds` / `_updates` / `_removes`; only ambiguous-identity or fabrication-risk cases route to `requires_authoring.notes`.
12. **Step ingredient link mismatches.** `recipe_step_ingredients` must be coherent with both the step instruction and the recipe-level ingredient list:
  ...
  - If the misrouted link is on a duplicate ingredient row (two `recipe_ingredients` rows sharing the same `ingredient_id`), also apply the section split from auto-check #23. The SQL `UPDATE` in `requires_authoring.notes` still has to fix the link itself — the YAML cannot mutate `recipe_step_ingredients.recipe_ingredient_id` because `(step + ingredient_slug)` cannot disambiguate two rows of the same ingredient — but the section split makes the underlying intent visible to the cook (e.g. Topping / Para servir for the frying oil vs Main / Principal for the sauté oil), so the duplicate stops reading as a bug while the link fix is queued for the admin.
...
20. **Spanish instruction voice — usted imperatives and Spanglish nouns.** Project convention is Mexican-Spanish **tú** with **canonical Spanish nouns** for kitchen items (see Irmixy personality and `system-prompt-builder.ts`). Both failure modes are closed-vocabulary and detectable mechanically; both fix via `step_text_overrides` (not `requires_authoring.notes`). Scan `recipe_step_translations.locale = 'es'` (and `es-ES` if present) `instruction`, `recipe_section`, and `tip` text against the violator vocabularies below.
...
21. **Step-ingredient quantity / unit drift.** Each `recipe_step_ingredients` row carries its own `quantity` and `measurement_unit_id`. When the row's `(recipe_step_id, ingredient_id)` resolves to exactly one recipe-level `recipe_ingredients` row (the common case — no duplicate ingredient rows on the recipe), the link's qty/unit should match the recipe-level row's values within rounding for unit conversions. When the recipe has duplicate `recipe_ingredients` rows for the same `ingredient_id`, no single recipe-level row is canonical for the link, so the drift check does not apply — handle the duplicate via auto-check #23 (section split) instead. Mismatch is almost always import drift (e.g. recipe-level cornstarch 30 g, step-level says 20 g). Walk every step ingredient against its recipe-level counterpart; for every drift, emit a `step_ingredient_updates` entry with the recipe-level value as the target. The data is in the snapshot under `recipe.steps[].step_ingredients[]` joined to `recipe.ingredients[]`; no extra query needed.
    ...
    **Route by failure mode:** quantity/unit drift on an existing-and-correct link goes here in `step_ingredient_updates`; mechanical wrong/missing/orphan links go to `step_ingredient_adds` / `_removes` under auto-check #12; only ambiguous-identity or fabrication-risk cases (the step contradicts itself, the ingredient identity is genuinely unclear, fixing would require inventing content) go to `requires_authoring.notes`.
...
- **Don't list capabilities the Thermomix already provides.** When `Thermomix®` is in the kitchen-tools set, omit tools that duplicate its built-in functions: `Kitchen scale` (built-in), `Kitchen timer` (built-in), `Blender jar` (the bowl is the blender), generic `Bowl` (redundant with Thermomix® — the simmering basket and Varoma are listed separately). Sibling YAMLs (`steamed-rice.yaml`, `mexican-rice.yaml`) have propagated `Kitchen scale` onto Thermomix-only recipes; that is drift, not precedent — do not propagate it. The valid Thermomix tool family for a typical recipe is: `Thermomix®`, plus any of `Thermomix® spatula`, `Thermomix® simmering basket`, `Thermomix® Varoma`, `Thermomix® Varoma tray`, `Cutter lid`, `Butterfly whisk` — only when the recipe actually uses them. List non-Thermomix tools (`Oven`, stovetop pan, baking tray, ovenproof dish, etc.) only when they are genuinely separate from the Thermomix.
...
- **Diet-tag attestation must match user-visible content.** Exclusion-style diet tags (`vegan`, `vegetarian`, `gluten_free`, `pescatarian`) are promises to the user, not loose categorization. Whenever you add or keep one of these tags, audit the recipe's description, `tips_and_tricks`, ingredient list, and step instructions for items that violate the promise. The hidden-violator pattern is the dangerous one — bouillon/stock tucked 8 ingredients deep, fish sauce as a background note, soy sauce as the wheat sneak in a "gluten-free" stir-fry. Walk the canonical violator vocabularies below explicitly, ingredient by ingredient.
  ...
  **Scope: diet tags identify what the dish IS, not every diet that COULD eat it.** Subset reasoning ("vegan ⊂ vegetarian ⊂ pescatarian, so apply all three") dilutes the filter for users searching specifically for fish/seafood or vegetarian-with-dairy dishes. Apply each tag based on the recipe's actual ingredient profile:

    - `vegan` — plant-only ingredients (no dairy, eggs, honey, gelatin, animal-derived flavorings).
    - `vegetarian` — apply alongside `vegan` when truly vegan (vegan ⊂ vegetarian); apply alone when dairy or eggs are present but no meat or seafood.
    - `pescatarian` — apply **only** when seafood is the protein (and no meat/poultry). Do **not** add `pescatarian` to vegan or vegetarian dishes just because pescatarians could eat them; users filtering for pescatarian want fish/seafood dishes specifically.
    - `gluten_free` — no wheat or gluten ingredients (run the violator sweep above; default-drop on uncertainty).
...
- **`recipe_step_ingredients` has no `recipe_ingredient_id` FK.** The link is by `(recipe_step_id, ingredient_id)` plus its own `quantity` / `measurement_unit_id` / `display_order` / `optional`. Recipe review snapshots derive a `recipe_ingredient_id` (and `recipe_ingredient_display_order`) field at export time by joining `(recipe_id, ingredient_id)` against `recipe_ingredients` for reviewer convenience. When duplicate `recipe_ingredients` rows share an `ingredient_id`, that derived field arbitrarily picks one match — the pick is not stored, not authoritative, and never the basis for a SQL `UPDATE`. Reviewers who see a "wrong" derived value should default to auto-check #23 (section split), not to fabricating a SQL fix against a non-existent column.
