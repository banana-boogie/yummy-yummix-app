...
10. **Step text vs structured Thermomix fields.** Step instructions specify temperature/speed in prose but the corresponding columns (`thermomix_speed`, `thermomix_temperature`) are NULL.
11. **Step references unknown ingredient.** Step instruction names an ingredient that is not in `recipe_ingredients`.
