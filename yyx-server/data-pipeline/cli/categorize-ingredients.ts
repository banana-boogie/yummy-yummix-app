...
import { callOpenAI } from '../lib/openai-client.ts';
...
async function categorizeBatch(
  ...
  if (!content) {
    logger.warn(`No content returned for batch of ${batch.length} — falling back to "other"`);
  ...
  for (const a of assignments) {
    ...
    if (!isValidCategory(a.category_id)) {
      logger.warn(`Invalid category "${a.category_id}" for id=${a.id} — using "other"`);
  ...
  for (const ing of batch) {
    if (!result.has(ing.id)) {
      logger.warn(`Missing assignment for id=${ing.id} (${ing.name_en}) — using "other"`);
...
function buildSqlFile(
  ...
  for (const { ingredient, category } of assignments) {
    const enComment = escapeSqlIdentifier(ingredient.name_en || ingredient.name_es || '');
    lines.push(
...
async function main() {
  ...
  const sql = buildSqlFile(all_assignments);
