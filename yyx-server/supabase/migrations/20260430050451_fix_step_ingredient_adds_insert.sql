-- Recipe Metadata Apply RPC — fix step_ingredient_adds INSERT.
--
-- Hotfix for the previous migration (20260430042650). That migration's
-- step_ingredient_adds INSERT referenced a `recipe_ingredient_id` column on
-- `recipe_step_ingredients` that does not exist on this DB. The function was
-- defined successfully (CREATE OR REPLACE doesn't validate the body), but
-- any first-use call would have failed at runtime. The Deno fetcher had the
-- mirror-image bug — selecting `recipe_ingredient_id` — which caused every
-- dry-run to fail before reaching apply.
--
-- Fix:
-- - Drop `recipe_ingredient_id` from the INSERT column list and VALUES.
-- - Drop the now-vestigial `SELECT id INTO v_recipe_ing_id` lookup; replace
--   it with an EXISTS check that just verifies a recipe-level row is
--   present (orphan-link prevention is unchanged conceptually — the
--   recipe-level row must exist, we just don't capture its id).
--
-- Forward-only CREATE OR REPLACE; everything else identical to 20260430042650.
--
-- (Original section docs preserved below for completeness.)
--
-- The link table (`recipe_step_ingredients`) carries a step's per-ingredient
-- usage with its own `quantity` and `measurement_unit_id`. Drift between this
-- table and the recipe-level `recipe_ingredients` row was the single biggest
-- driver of `requires_authoring.notes` admin SQL handoffs (auto-checks #12
-- and #21 in `docs/agent-guidelines/RECIPE-REVIEW.md`). Pulling those
-- mutations into the YAML applier collapses the round-trip — reviewers
-- express the fix as a YAML section, the apply RPC handles it
-- transactionally with the rest of the recipe edit.
--
-- New payload sections:
-- - step_ingredient_updates: edit `quantity` / `measurement_unit_id` on an
--   existing link. Match by (step_id|order) + ingredient_slug. Each YAML
--   field is optional; null-valued fields explicitly clear the column.
--   IS DISTINCT FROM gating; unchanged YAML produces zero writes.
-- - step_ingredient_adds: insert a missing link. Match step by
--   (step_id|order), identify the ingredient by `ingredient_slug`. The
--   recipe-level `recipe_ingredients` row MUST exist (orphan-link
--   prevention) — the RPC hard-fails if not, with a message pointing the
--   reviewer at `ingredient_adds` to bootstrap the recipe-level row first.
--   Idempotent: re-applying with an existing link no-ops.
...
-- three new keys. All other behavior is preserved.
--
-- Originally documented capabilities (still in effect):
-- - step_text_overrides: per-locale upsert into recipe_step_translations
--   (instruction, recipe_section, tip). Match by step_id (preferred) or
--   order. Per-locale upsert with IS DISTINCT FROM gating; unchanged
--   YAML produces zero writes. Each field is COALESCE'd against current
--   state so omitting fields leaves them untouched. The applier never
--   deletes a translation row.
...
DECLARE
  ...
  v_step_text_done       int := 0;
  v_step_ing_updates     int := 0;
  v_step_ing_adds        int := 0;
  v_step_ing_removes     int := 0;
  v_tags_added           int := 0;
...
BEGIN
  ...
  IF payload ? 'planner' THEN
    ...
    UPDATE public.recipes r
       SET planner_role = COALESCE(v_section->>'role', r.planner_role),
           ...
           -- is_published intentionally omitted; admin-only, RPC ignores it.
     WHERE r.id = v_recipe_id
       AND (
         ...
         -- is_published omitted from gating; RPC ignores reviewer payload.
       );
  ...
  -- step_text_overrides — per-locale upsert into recipe_step_translations
  --
  -- New in this migration. Lets the YAML mutate the per-locale text
  -- (instruction / recipe_section / tip) on a recipe_step. Match key is
  -- step_id (preferred) or order. Each YAML field is COALESCE'd against
  -- current state so omitted fields stay untouched. IS DISTINCT FROM
  -- gating means an unchanged YAML produces zero writes.
  --
  -- Schema constraint check: recipe_step_translations.instruction is NOT
  -- NULL. The YAML schema rejects an empty string at parse time, and
  -- bootstrap-without-instruction is rejected here so we don't violate
  -- the column constraint with a mid-flight error.
  -- ------------------------------------------------------------
  IF payload ? 'step_text_overrides' THEN
    FOR v_section IN SELECT * FROM jsonb_array_elements(payload->'step_text_overrides') LOOP
      v_step_id := NULL;
      v_match := v_section -> 'match';
      ...
      IF v_step_id IS NULL THEN
        RAISE EXCEPTION 'step_text_overrides: no recipe_step match for %', v_match::text;
      END IF;
      ...
      FOR v_locale IN SELECT unnest(ARRAY['en','es']) LOOP
        DECLARE
          ...
          v_n                int;
        BEGIN
          v_block := v_section #> ARRAY['translations', v_locale];
          IF v_block IS NULL THEN CONTINUE; END IF;

          v_inst_new    := v_block ->> 'instruction';
          v_section_new := v_block ->> 'recipe_section';
          v_tip_new     := v_block ->> 'tip';

          -- Schema requires at least one of these to be set, but be
          -- defensive in case the payload was hand-crafted past the schema.
          IF v_inst_new IS NULL AND v_section_new IS NULL AND v_tip_new IS NULL THEN
            CONTINUE;
          END IF;

          SELECT instruction, recipe_section, tip
            INTO v_inst_current, v_section_current, v_tip_current
            FROM public.recipe_step_translations
            WHERE recipe_step_id = v_step_id AND locale = v_locale;

          IF NOT FOUND THEN
            -- Bootstrap a new translation row. instruction is NOT NULL on
            -- the column, so refuse if the YAML didn't provide it.
            IF v_inst_new IS NULL THEN
              RAISE EXCEPTION
                'step_text_overrides: no % translation row for step %; YAML must provide translations.%.instruction to bootstrap',
                v_locale, v_step_id, v_locale;
            END IF;
            INSERT INTO public.recipe_step_translations
              (recipe_step_id, locale, instruction, recipe_section, tip)
              VALUES (v_step_id, v_locale, v_inst_new, v_section_new, v_tip_new);
            v_step_text_done := v_step_text_done + 1;
            v_changed := true;
          ELSE
            UPDATE public.recipe_step_translations rst
              SET instruction    = COALESCE(v_inst_new, rst.instruction),
                  recipe_section = COALESCE(v_section_new, rst.recipe_section),
                  tip            = COALESCE(v_tip_new, rst.tip)
              WHERE rst.recipe_step_id = v_step_id
                AND rst.locale = v_locale
                AND (
                  COALESCE(v_inst_new, rst.instruction) IS DISTINCT FROM rst.instruction
                  OR COALESCE(v_section_new, rst.recipe_section) IS DISTINCT FROM rst.recipe_section
                  OR COALESCE(v_tip_new, rst.tip) IS DISTINCT FROM rst.tip
                );
            GET DIAGNOSTICS v_n = ROW_COUNT;
            IF v_n > 0 THEN
              v_step_text_done := v_step_text_done + 1;
  ...
  -- ------------------------------------------------------------
  -- step_ingredient_updates — edit quantity / measurement_unit_id on an
  -- existing recipe_step_ingredients row.
  --
  ...
  -- zero writes.
  -- ------------------------------------------------------------
  IF payload ? 'step_ingredient_updates' THEN
    FOR v_section IN SELECT * FROM jsonb_array_elements(payload->'step_ingredient_updates') LOOP
      v_step_id := NULL;
      ...
      IF v_step_id IS NULL THEN
        RAISE EXCEPTION 'step_ingredient_updates: no recipe_step match for %', v_match::text;
      END IF;
      ...
      IF v_ingredient_id IS NULL THEN
        RAISE EXCEPTION
          'step_ingredient_updates: no ingredient with slugified name_en = %',
          v_match_slug;
      END IF;
  ...
  -- ------------------------------------------------------------
  -- step_ingredient_adds — insert a recipe_step_ingredients row.
  --
  -- Orphan-link prevention: the recipe-level recipe_ingredients row for
  -- this (recipe, ingredient) MUST already exist. The link table has no
  -- FK column to recipe_ingredients on this DB; the binding is by
  -- ingredient_id alone. "Orphan" is conceptually defined as "no
  -- recipe_ingredients row exists for this (recipe, ingredient)". Hard-
  -- fail with a message pointing the reviewer at ingredient_adds.
  --
  ...
  IF payload ? 'step_ingredient_adds' THEN
    FOR v_section IN SELECT * FROM jsonb_array_elements(payload->'step_ingredient_adds') LOOP
      ...
      IF v_step_id IS NULL THEN
        RAISE EXCEPTION 'step_ingredient_adds: no recipe_step match for %', v_match::text;
      END IF;
      ...
      IF v_ingredient_id IS NULL THEN
        RAISE EXCEPTION
          'step_ingredient_adds: no ingredient with slugified name_en = %; create it in the admin UI first',
          v_match_slug;
      END IF;

      -- Orphan-link prevention: must have a recipe-level row. We don't
      -- need to capture the row's id — recipe_step_ingredients has no FK
      -- column to recipe_ingredients on this DB; the link is by
      -- ingredient_id alone and "orphan" is conceptually defined as
      -- "no recipe_ingredients row exists for this (recipe, ingredient)".
      IF NOT EXISTS (
        SELECT 1 FROM public.recipe_ingredients
         WHERE recipe_id = v_recipe_id AND ingredient_id = v_ingredient_id
      ) THEN
        RAISE EXCEPTION
          'step_ingredient_adds: ingredient_slug=% has no recipe_ingredients row on recipe % — add it via ingredient_adds first to avoid an orphan link',
          v_match_slug, v_recipe_id;
      END IF;

      -- Idempotent: skip if a link already exists for this (step, ingredient).
      IF EXISTS (
      ...
      INSERT INTO public.recipe_step_ingredients
        (recipe_step_id, ingredient_id,
         display_order, quantity, measurement_unit_id)
        VALUES (
          v_step_id,
          v_ingredient_id,
      ...
      v_step_ing_adds := v_step_ing_adds + 1;
      v_changed := true;
    END LOOP;
  END IF;

  -- ------------------------------------------------------------
  -- step_ingredient_removes — drop a recipe_step_ingredients row.
  -- Match: (step_id | order) + ingredient_slug. Idempotent — an absent
  -- link no-ops (DELETE returns zero rows).
  -- ------------------------------------------------------------
  IF payload ? 'step_ingredient_removes' THEN
    FOR v_section IN SELECT * FROM jsonb_array_elements(payload->'step_ingredient_removes') LOOP
      v_step_id := NULL;
      v_ingredient_id := NULL;
      v_match := v_section -> 'match';
      ...
      -- Step gone? Idempotent — nothing to remove.
      IF v_step_id IS NULL THEN CONTINUE; END IF;
      ...
      IF FOUND THEN
        v_step_ing_removes := v_step_ing_removes + 1;
        v_changed := true;
      END IF;
    END LOOP;
  END IF;
  ...
  v_counts := jsonb_build_object(
    ...
    'step_text_overrides', v_step_text_done,
    'step_ingredient_updates', v_step_ing_updates,
    'step_ingredient_adds', v_step_ing_adds,
    'step_ingredient_removes', v_step_ing_removes,
