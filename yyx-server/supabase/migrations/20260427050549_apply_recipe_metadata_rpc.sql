-- Recipe Metadata Apply RPC
--
-- The transactional apply path for the recipe-metadata pipeline (Plan 12).
-- The Deno CLI at yyx-server/data-pipeline/cli/apply-recipe-metadata.ts
-- builds a JSONB payload from a YAML config, validates it client-side, and
-- calls this function. Every section is applied inside the function's
-- implicit transaction — if any section fails, the whole apply rolls back.
--
-- Contract:
--   apply_recipe_metadata(payload jsonb) RETURNS jsonb
--   { ok: bool, counts: { ... }, errors: [ ... ] }
--
-- Stale-diff guard: payload.recipe_match.expected_recipe_updated_at must
-- match the row's current updated_at (or be ahead, in case writes happened
-- on this same connection). If the DB row has advanced past the expected
-- timestamp, the function raises `stale_diff` and the caller must re-run
-- the Reviewer to refresh state.
--
-- Idempotency: every section uses IS DISTINCT FROM gating, declarative
-- set-replacement diffs, or composite-key existence checks. Re-running an
-- unchanged payload produces zero data rows touched (counts all zero).

-- ============================================================
-- 1. Slugify helper
--
-- Used to derive a stable identifier from `ingredients.name_en` for
-- ingredient match keys. Mirrors what the Reviewer computes client-side.
-- Scoped private (`_recipe_metadata_*`) so it does not collide with any
-- future general-purpose slugify.
-- ============================================================

CREATE OR REPLACE FUNCTION public._recipe_metadata_slugify(input text)
RETURNS text
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
COMMENT ON FUNCTION public._recipe_metadata_slugify(text) IS
  'Internal helper for apply_recipe_metadata. Derives a stable lowercase '
  'snake-case slug from a free-form name. Strips diacritics for common '
  'Spanish/European characters. Used to match recipe_ingredients by '
  '(slugified name_en, display_order) when no UUID is supplied.';

-- ============================================================
-- 2. apply_recipe_metadata
-- ============================================================
...
DECLARE
  ...
  v_pair_desired_keys    text[];   -- 'target_id|role' tuples
...
BEGIN
  ...
  -- name_en is a readability assertion. If it does not match the live row,
  -- the YAML is almost certainly pointed at the wrong recipe (rename or
  -- copy/paste error) — fail loudly rather than mutate the wrong row.
  ...
  --
  -- Each is per-locale upsert into recipe_translations. INSERT requires
  -- a `name` (NOT NULL); if the recipe has no row for that locale yet,
  -- the YAML must include `name.<locale>` to bootstrap.
  -- ------------------------------------------------------------
  FOR v_locale IN SELECT unnest(ARRAY['en','es']) LOOP
    ...
    BEGIN
      ...
      SELECT true, name, description, tips_and_tricks, scaling_notes
        INTO v_row_exists, v_name_current, v_desc_current, v_tips_current, v_scaling_current
  ...
  IF payload ? 'ingredient_updates' THEN
    FOR v_section IN SELECT * FROM jsonb_array_elements(payload->'ingredient_updates') LOOP
      ...
      -- Per-locale notes update on recipe_ingredient_translations
      FOR v_locale IN SELECT unnest(ARRAY['en','es']) LOOP
        DECLARE
          v_notes_new   text;
          v_section_key text;
  ...
  -- ------------------------------------------------------------
  -- ingredient_adds
  -- Resolve ingredient_slug → ingredients.id via slugify(name_en).
  -- Refuses if no match — the Reviewer must not auto-create ingredients.
  -- ------------------------------------------------------------
  IF payload ? 'ingredient_adds' THEN
    FOR v_section IN SELECT * FROM jsonb_array_elements(payload->'ingredient_adds') LOOP
      ...
      -- Skip if a recipe_ingredient already exists at that display_order
      -- with the same ingredient_id (idempotency).
      IF EXISTS (
      ...
      -- Insert translation rows for any provided notes/sections.
      INSERT INTO public.recipe_ingredient_translations
        (recipe_ingredient_id, locale, notes, recipe_section)
  ...
  IF payload ? 'kitchen_tools' THEN
    ...
    FOR v_kt_entry IN SELECT * FROM jsonb_array_elements(payload #> '{kitchen_tools,set}') LOOP
      ...
      DECLARE
        v_kt_match_count int;
      BEGIN
        SELECT count(*), max(kt.id) INTO v_kt_match_count, v_kt_id
          FROM public.kitchen_tools kt
          JOIN public.kitchen_tool_translations ktt
            ON ktt.kitchen_tool_id = kt.id AND ktt.locale = 'en'
         WHERE lower(trim(ktt.name)) = lower(trim(v_kt_lookup));
        IF v_kt_match_count > 1 THEN
          RAISE EXCEPTION
            'kitchen_tools: ambiguous name_en "%" matches % rows — disambiguate in the admin UI',
            v_kt_lookup, v_kt_match_count;
        END IF;
        IF v_kt_match_count = 0 THEN v_kt_id := NULL; END IF;
      END;
    ...
    -- Added = desired not in current; insert with sequential display_order
    -- starting after the highest existing.
    DECLARE
      v_next_order int;
      v_kt_idx     int;
    BEGIN
      ...
      v_kt_idx := 1;
      WHILE v_kt_idx <= array_length(v_kt_desired, 1) LOOP
    ...
    -- Notes upsert per entry (by name lookup, locale-aware)
    FOR v_kt_entry IN SELECT * FROM jsonb_array_elements(payload #> '{kitchen_tools,set}') LOOP
      v_kt_lookup := v_kt_entry ->> 'name_en';
      DECLARE
        v_kt_match_count int;
  ...
  -- ------------------------------------------------------------
  -- pairings (declarative set, composite key target_id+role)
  -- ------------------------------------------------------------
  IF payload ? 'pairings' THEN
    ...
    -- Remove pairings absent from desired
    ...
    -- Insert pairings new to desired
    FOR v_pair_entry IN SELECT * FROM jsonb_array_elements(payload #> '{pairings,set}') LOOP
      v_pair_key := (v_pair_entry->>'target_id') || '|' || (v_pair_entry->>'role');
    ...
    FOR v_section IN SELECT * FROM jsonb_array_elements(payload->'step_overrides') LOOP
      ...
      UPDATE public.recipe_steps s
        SET thermomix_time = CASE WHEN v_section ? 'thermomix_time'
            ...
            thermomix_speed = CASE WHEN v_section ? 'thermomix_speed'
              THEN v_section->>'thermomix_speed'
              ELSE s.thermomix_speed END,
            thermomix_speed_start = CASE WHEN v_section ? 'thermomix_speed_range'
              THEN v_section #>> '{thermomix_speed_range,start}'
              ELSE s.thermomix_speed_start END,
            thermomix_speed_end = CASE WHEN v_section ? 'thermomix_speed_range'
              THEN v_section #>> '{thermomix_speed_range,end}'
              ELSE s.thermomix_speed_end END,
            thermomix_temperature = CASE WHEN v_section ? 'thermomix_temperature'
              THEN v_section->>'thermomix_temperature'
              ELSE s.thermomix_temperature END,
            thermomix_temperature_unit = CASE WHEN v_section ? 'thermomix_temperature_unit'
              THEN v_section->>'thermomix_temperature_unit'
        ...
        WHERE s.id = v_step_id
          AND (
            ...
            OR (v_section ? 'thermomix_speed' AND v_section->>'thermomix_speed' IS DISTINCT FROM s.thermomix_speed)
            OR (v_section ? 'thermomix_speed_range' AND
                ((v_section #>> '{thermomix_speed_range,start}') IS DISTINCT FROM s.thermomix_speed_start
                 OR (v_section #>> '{thermomix_speed_range,end}') IS DISTINCT FROM s.thermomix_speed_end))
            OR (v_section ? 'thermomix_temperature' AND v_section->>'thermomix_temperature' IS DISTINCT FROM s.thermomix_temperature)
            OR (v_section ? 'thermomix_temperature_unit' AND v_section->>'thermomix_temperature_unit' IS DISTINCT FROM s.thermomix_temperature_unit)
  ...
  -- Only categories present in YAML are diffed; absent categories untouched.
  -- ------------------------------------------------------------
  IF payload ? 'tags' THEN
    FOR v_tag_category IN SELECT unnest(ARRAY['cuisine','meal_type','diet','occasion','practical']) LOOP
      ...
      -- Resolve all desired slugs to IDs and validate category membership
      DECLARE
        v_desired_ids uuid[];
        v_id          uuid;
        v_n           int;
      BEGIN
        ...
        -- Delete tags that are current-but-not-desired (within this category)
        ...
        -- Insert tags that are desired-but-not-current
  ...
  -- ------------------------------------------------------------
  -- Bump updated_at on the recipe row when any section touched data.
  -- Independent of any handle_updated_at trigger, so the stale-diff
  -- guard works on the next apply regardless of trigger state.
  -- ------------------------------------------------------------
  IF v_changed THEN
...
COMMENT ON FUNCTION public.apply_recipe_metadata(jsonb) IS
  'Transactionally apply a recipe-metadata diff produced by the Plan 12 '
  'Reviewer. Stale-diff-guarded via expected_recipe_updated_at. Idempotent: '
  're-running an unchanged payload produces zero data writes.';

-- ============================================================
-- 3. Permissions
--
-- SECURITY DEFINER means the function runs with the owning role's
-- privileges. We REVOKE PUBLIC and GRANT only to service_role to make
-- sure the anon and authenticated roles cannot trigger arbitrary
-- recipe mutations.
-- ============================================================
