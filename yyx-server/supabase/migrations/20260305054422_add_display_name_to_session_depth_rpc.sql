-- Add display name to top_users in admin_ai_chat_session_depth
-- Joins user_profiles to resolve name (or username fallback) + last 4 of UUID
  ...
  top_users AS (
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'displayName', COALESCE(up.name, up.username, 'Unknown'),
