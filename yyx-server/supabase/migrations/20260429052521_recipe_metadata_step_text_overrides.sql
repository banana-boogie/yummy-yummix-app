-- Recipe Metadata Apply RPC — add step_text_overrides support.
--
-- Adds a new payload section `step_text_overrides` that lets the YAML
-- mutate per-locale text on `recipe_step_translations` (instruction,
-- recipe_section, tip). Pre-existing review notes pushed step text
-- corrections to admin authoring even when the fix was a clean rewrite
-- (e.g. usted→tú, gendered noun agreement); the RPC didn't expose the
-- table so YAML reviewers had no choice. With this migration, the
-- review-recipe skill can express those edits directly.
--
-- Idempotency / safety:
-- - Match the row by step_id (preferred) or order; resolve the step UUID
--   first so the upsert is bound to the right recipe_step row.
-- - Per-locale upsert with IS DISTINCT FROM gating; an unchanged YAML
--   produces zero writes.
-- - Each YAML field is COALESCE'd against current state so omitting
--   `recipe_section` or `tip` leaves them untouched (only fields the YAML
--   sets are diffed and updated).
-- - The applier never deletes a translation row. Reviewer can clear text
--   in a follow-up: not a v1 concern (use cleanup.delete_locales for
--   wholesale locale removal, or admin UI for surgical clears).
--
-- Out of scope: recipe_step_ingredients mutation. Relationship-row
-- cleanup is higher-risk and gets its own design pass.
