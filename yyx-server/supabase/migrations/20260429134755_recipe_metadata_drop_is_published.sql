-- Recipe Metadata Apply RPC — drop is_published from planner UPDATE.
--
-- Publishing is admin-only. The reviewer YAML schema rejects
-- `planner.is_published` (see `recipe-metadata-schema.ts` plannerSchema
-- comment), but the RPC was still writing the field via COALESCE if a
-- direct service-role caller smuggled it in the payload. Belt-and-
-- suspenders: this migration replaces the function so the planner
-- UPDATE no longer touches `is_published` at all — even if a future
-- caller bypasses the schema, the RPC silently ignores any
-- `is_published` value in the planner section.
--
-- Forward-only CREATE OR REPLACE; functionally identical to the
-- previous step_text_overrides migration except the planner UPDATE and
-- its IS DISTINCT FROM gating no longer reference `is_published`. All
...
--
-- Out of scope: recipe_step_ingredients mutation.
