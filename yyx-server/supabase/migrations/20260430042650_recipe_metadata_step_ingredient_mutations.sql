-- Recipe Metadata Apply RPC — add recipe_step_ingredients mutations.
--
...
BEGIN
  ...
  -- this (recipe, ingredient) MUST already exist. The schema's FK on
  -- recipe_ingredient_id is nullable, but creating step-level links to
  -- ingredients absent from the recipe-level list is the exact failure
  ...
  IF payload ? 'step_ingredient_adds' THEN
    FOR v_section IN SELECT * FROM jsonb_array_elements(payload->'step_ingredient_adds') LOOP
      ...
      -- Orphan-link prevention: must have a recipe-level row.
      SELECT id INTO v_recipe_ing_id
        FROM public.recipe_ingredients
      ...
      INSERT INTO public.recipe_step_ingredients
        (recipe_step_id, ingredient_id, recipe_ingredient_id,
