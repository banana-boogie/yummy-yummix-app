...
--
-- The embedding column stores provider-agnostic vectors (3072 float dimensions).
-- "text-embedding-3-large" is the model name used for generation, but the column
-- itself is vendor-neutral — any model producing 3072-dim vectors works.
--
-- Note: pgvector extension is in the 'extensions' schema (Supabase default).
-- HNSW max is 2000 dims for vector, 4000 for halfvec — use halfvec cast for index.
