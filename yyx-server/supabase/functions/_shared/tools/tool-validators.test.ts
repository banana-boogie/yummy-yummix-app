...
Deno.test('sanitizeString rejects non-string types', () => {
  assertThrows(() => sanitizeString(123, 100), ToolValidationError, 'Expected string');
  assertThrows(() => sanitizeString({}, 100), ToolValidationError, 'Expected string');
  assertThrows(() => sanitizeString([], 100), ToolValidationError, 'Expected string');
});
...
Deno.test('validateUUID rejects invalid formats', () => {
  assertThrows(() => validateUUID('not-a-uuid'), ToolValidationError, 'Invalid UUID');
  assertThrows(() => validateUUID('a1b2c3d4e5f67890abcdef1234567890'), ToolValidationError);
...
Deno.test('validateSearchRecipesParams clamps maxTime to valid range', () => {
  ...
  const tooHigh = validateSearchRecipesParams({ maxTime: 1000, cuisine: 'any' });
...
Deno.test('validateSearchRecipesParams accepts valid difficulty enum', () => {
  assertEquals(validateSearchRecipesParams({ difficulty: 'easy', cuisine: 'any' }).difficulty, 'easy');
  assertEquals(validateSearchRecipesParams({ difficulty: 'medium', cuisine: 'any' }).difficulty, 'medium');
  assertEquals(validateSearchRecipesParams({ difficulty: 'hard', cuisine: 'any' }).difficulty, 'hard');
});
...
Deno.test('validateSearchRecipesParams handles JSON string input', () => {
  const params = validateSearchRecipesParams('{"cuisine": "Mexican", "maxTime": 45}');
...
Deno.test('validateSearchRecipesParams handles SQL injection attempts', () => {
  ...
  const params1 = validateSearchRecipesParams({ query: "'; DROP TABLE recipes;--" });
  assertEquals(params1.query?.includes(';'), true); // We only strip commas, SQL is handled by parameterized queries

  // Commas are stripped
  const params2 = validateSearchRecipesParams({ query: "test, SELECT * FROM users" });
