...
import { validateRecipeData, normalizeRecipeData } from '../recipe-validator.ts';
...
Deno.test('validateRecipeData - invalid difficulty value returns error', () => {
  ...
  assertEquals(result.valid, false);
  assertEquals(result.errors.some((e) => e.includes('easy') || e.includes('medium') || e.includes('hard')), true);
});
