...
Deno.test("renderSelectionReason: es-MX resolves to Spanish templates", () => {
  const text = renderSelectionReason("no_cook_fallback", "es-MX", {
    dayLabel: "miércoles",
  });
  assertStringIncludes(text, "sin cocinar");
...
Deno.test("getNoCookPlaceholderTitle: locale-specific title", () => {
  assertEquals(getNoCookPlaceholderTitle("en"), "No-cook meal");
  assertEquals(getNoCookPlaceholderTitle("es"), "Comida sin cocinar");
  assertEquals(getNoCookPlaceholderTitle("es-MX"), "Comida sin cocinar");
});
