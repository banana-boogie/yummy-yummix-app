...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = url.searchParams.get('systemPrompt') || 'You are a helpful voice assistant.';
      ...
      try {
        ...
        sttProvider.onUtteranceEnd(async () => {
          ...
          try {
            ...
            const { inputTokens, outputTokens } = await llmProvider.streamResponse(
              systemPrompt,
              conversationHistory,
              async (sentence) => {
            ...
            console.log(`[HTS] Turn complete - Tokens: ${inputTokens} in, ${outputTokens} out`);
          } catch (error) {
    ...
    async function cleanupSession() {
      try {
        ...
        const llmCost = (llmTokensInput * 0.15 / 1_000_000) + (llmTokensOutput * 0.60 / 1_000_000);
        ...
        console.log(
          ...
          `  LLM cost: $${llmCost.toFixed(6)} (${llmTokensInput} in, ${llmTokensOutput} out)\n` +
          `  TTS cost: $${ttsCost.toFixed(6)} (${ttsCharacterCount} chars)\n` +
