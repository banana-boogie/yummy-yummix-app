...
**Consult sibling YAMLs for tag-policy precedent.** The rubric's tag bars are intentionally strict, and the catalog has accumulated case-specific precedent that can clarify "should this borderline tag apply?" in a given dish. Before settling contested tag decisions (`kid_friendly`, `vegetarian`/`vegan`, `cuisine` on generic dishes, `one_pot`, `make_ahead`, `comfort_food`), skim 1-2 already-reviewed sibling YAMLs in `yyx-server/data-pipeline/data/recipe-metadata/` for similar dishes — pick by ingredient family or planner role. The YAML body shows what was kept; the report-time judgment lives in the commit message and user feedback the reviewer can't see. If a sibling chose conservatively where you're tempted to add, default to the sibling's choice and flag the divergence in the Step 7 ▸ Skipped on purpose bucket so the user can ratify either way. (Long-term, those policies belong in `RECIPE-REVIEW.md` itself; until then, sibling YAMLs are the working precedent.)
