export const recipes = {
  sections: {
    quick: "Cenas Rápidas Entre Semana",
    forYou: "Solo para Ti",
    family: "Favoritos de la Familia",
    new: "Nuevo Esta Semana",
    all: "Todas las Recetas",
    todays_meal: "La Comida de Hoy",
    for_you: "Para Ti",
    favourites: "Tus Favoritos",
    quick_easy: "Rápido y Fácil",
    worth_a_try: "Vale la Pena",
    popular: "Popular",
  },
  filters: {
    quick: "Rápido",
    easy: "Fácil",
    gluten_free: "Sin Gluten",
    vegetarian: "Vegetariano",
    vegan: "Vegano",
    mexican: "Mexicano",
    italian: "Italiano",
    asian: "Asiático",
    mediterranean: "Mediterráneo",
    desserts: "Postres",
    air_fryer: "Freidora de Aire",
  },
  addToPlan: {
    title: "Agregar al Plan",
    noPlanPrompt: "Aún no tienes un plan activo. ¿Creamos uno?",
    createPlanCta: "Crear un plan",
    selectSlot: "Elige el día y la comida",
    successToast: "Agregado a tu plan",
    errorToast: "No se pudo agregar al plan. Inténtalo de nuevo.",
    cancel: "Cancelar",
    weekComingSoon: "La pestaña Semana llegará pronto",
  },
  header: {
    greeting: "¡Hola, {{name}}!",
    subtitle: "¿Qué cocinamos hoy?",
  },
  common: {
    search: "¿Qué se te antoja comer?",
    loading: "Cargando recetas...",
    error: "Error al cargar las recetas",
    portions: "Porciones",
    prepTime: "Prep",
    totalTime: "Total",
    difficultyLabel: "Dificultad",
    ingredients: "Ingredientes",
    instructions: "Instrucciones",
    steps: "pasos",
    difficulty: {
      easy: "Fácil",
      medium: "Medio",
      hard: "Difícil",
    },
    emptyState: "No se encontraron recetas",
    noRecipesFound: "No se encontraron recetas",
    noResultsIrmixyPrompt: "¿No encuentras lo que buscas? ¡Te puedo ayudar!",
    askIrmixy: "Pregunta a Irmixy",
    kitchenTools: "Utensilios de Cocina",
    servings: "porciones",
    cookingMode: {
      slow_cook: "Cocción Lenta",
      rice_cooker: "Arrocera",
      sous_vide: "Sous Vide",
      fermentation: "Fermentación",
      open_cooking: "Cocción Abierta",
      browning: "Dorar",
      dough: "Masa",
      turbo: "Turbo",
    },
  },
  detail: {
    ingredients: {
      heading: "Ingredientes",
      main: "Ingredientes Principales",
      optional: "opcional",
      quantity: '{{amount}} {{unit}}{{connector ? " " + connector : ""}}',
    },
    steps: {
      heading: "Procedimiento",
      section: "Sección",
      parameters: {
        time: {
          minutes: "{{count}} mins.",
          seconds: "{{count}} seg.",
        },
        speed: "vel. {{speed}}",
        reversed: "giro a la izquierda",
      },
    },
    tips: "Consejos",
    kitchenTools: {
      heading: "Utensilios de Cocina",
      notes: "Notas",
    },
  },
  cookingGuide: {
    subtitle: "Mise en place",
    start: "¡Cocinar hoy!",
    intro: {
      miseEnPlace: {
        one: "Hagamos nuestro",
        two: "\nmise en place\n",
      },
      checkboxSteps: {
        checkmark: "Dale palomita ",
        steps:
          "a cada ingrediente que tengas listo, y luego haz click en 'siguiente' para el próximo paso.",
      },
    },
    navigation: {
      step: "Paso {{step}} de {{total}}",
      finish: "¡Terminar!",
      next: "Siguiente",
      back: "Atrás",
      askIrmixy: "Preguntarle a Irmixy",
      exitCookingGuide: "Salir de la guía de cocina",
      exit: "Salir",
      needHelp: "¿Necesitas ayuda?",
      irmixyLabel: "Irmixy",
    },
    miseEnPlace: {
      ingredients: {
        heading: "Ingredientes",
      },
      kitchenTools: {
        heading: "Utensilios de Cocina",
      },
    },
    kitchenTimer: "Temporizador",
    timerStart: "Iniciar",
    timerPause: "Pausar",
    timerReset: "Reiniciar",
    timerDone: "¡Tiempo!",
  },
  share: {
    message: "¡Mira esta receta en YummyYummix!",
  },
};
