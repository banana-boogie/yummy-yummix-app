...
**Consult sibling YAMLs for tag-policy precedent — tags only, not kitchen tools.** The rubric's tag bars are intentionally strict and the catalog has accumulated case-specific precedent that can clarify "should this borderline tag apply?" in a given dish. Before settling contested **tag** decisions (`kid_friendly`, `vegetarian`/`vegan`/`pescatarian` per the diet-tag scope rule in `RECIPE-REVIEW.md`, `cuisine` on generic dishes, `one_pot`, `make_ahead`, `comfort_food`), skim 1-2 already-reviewed sibling YAMLs in `yyx-server/data-pipeline/data/recipe-metadata/` for similar dishes — pick by ingredient family or planner role. If a sibling chose conservatively where you're tempted to add, default to the sibling's choice and flag the divergence in the Step 7 ▸ Skipped on purpose bucket so the user can ratify either way.

**Do NOT use sibling YAMLs as kitchen-tool precedent.** Kitchen-tool sets in the catalog have known drift — `Kitchen scale` and other Thermomix-built-in capabilities have been propagated onto Thermomix-only recipes that don't need them. Apply the explicit rule in `RECIPE-REVIEW.md` ("Don't list capabilities the Thermomix already provides") instead; let the rubric, not sibling YAMLs, settle tool-set decisions.

**The user is the tiebreaker on tags when the rubric is silent.** Sibling YAMLs are working precedent, not authoritative — they were each authored under the same review pressure. When the rubric in `RECIPE-REVIEW.md` doesn't explicitly settle a tag call, surface the call in the Step 7 ▸ Skipped on purpose or ▸ Routine bucket with a one-line reason, and let the user ratify or override. Don't pretend sibling agreement = correctness.

## Step 5 — Write the YAML

**Refresh-mode guardrail — tri-state on file existence + `applied:` block.** Before writing, check both whether a YAML at `yyx-server/data-pipeline/data/recipe-metadata/<slug>.yaml` is already on disk *and* whether it has a non-empty `applied:` block:

- **No file on disk** → first-write mode. Write everything from live state.
- **File exists, no `applied:` block** → first-write mode. The body was authored by a previous reviewer but never landed on the DB; trust the live state, not the YAML. Discard the prior body and decide every section from scratch. Only the `recipe_match` and `review` blocks survive (and `review.reviewed_*` get rewritten to your run).
- **File exists, has `applied:` block** → real refresh mode. The YAML reflects state that was once applied to the DB, so it's a useful starting point — but the recipe may have drifted since (re-imports, admin edits, applies from sibling worktrees). Do **not** blindly trust the body. Compare every section against current DB state field-by-field.

In real refresh mode:
...
**Sweep stale comments before the dry-run, not after.** When you revise a tag/role/section decision mid-review (e.g. user pushes back on a tag and you drop it), immediately delete or update the comment block that justified the prior decision in the same edit. Stale rationale comments outlive the body change they explained and confuse the next reviewer. The discipline: every YAML edit closes its own comment trail before the next dry-run runs.

**Content preservation: copy text verbatim from live state. Edits require explicit authorization.** When emitting `description.en/es`, `tips_and_tricks.en/es`, `scaling_notes.en/es`, `step_text_overrides`, or any user-facing prose, copy the live DB content character-for-character — including punctuation, units, em-dashes vs hyphens, straight vs curly apostrophes, capitalization, abbreviations (`seg`, `vel`), and trailing periods. The **only** edits to user-facing text are those a specific auto-check or judgment-call bullet authorizes (auto-checks #10/#11/#19/#20, the diet-tag content audit, voice fixes, etc.). If you find yourself "cleaning up" text that no rule flagged — converting `2 inches` to `5 cm`, inserting an em-dash for parallelism, normalizing punctuation, fixing a stray space — **stop and revert**. Same discipline as the YAML-comments rule: if it's not authorized, it doesn't belong in the diff. Drift introduced this way is invisible at apply (the dry-run shows it as a normal change, not a defect) and survives in production until someone re-reads the live recipe and notices.

Match-key rules:

**Slug format: `lower_snake_case`.** Every `ingredient_slug` in the YAML must match `^[a-z0-9][a-z0-9_]*$` — lowercase letters, digits, and underscores only. Hyphens are rejected by `.strict()` parsing at write time. Worked examples: `black_pepper` (not `black-pepper`), `extra_virgin_olive_oil`, `all_purpose_flour`. The slug derives from the EN ingredient name via `slugifyName()` (`data-pipeline/lib/recipe-metadata-fetch.ts`) — accents stripped, non-alphanumerics collapsed to `_`, trimmed.

- `ingredient_updates`/`ingredient_removes`: prefer durable `ingredient_slug` + `display_order` because `recipe_ingredients.id` values can rotate on re-import. Use `existing_id` only when a slug is unavailable or the row cannot be disambiguated by slug + display order. Never use names directly.
...
**If the dry-run errors for a non-YAML reason — stop and surface the error; do not deliver triaged buckets.** "Non-YAML reason" means the pipeline itself failed before producing a diff: a Postgres column-not-found, a fetcher timeout, a permissions error, an RPC migration that hasn't landed, an out-of-date Supabase CLI session. The triage report's bucketing logic exists to ratify a *verified* diff — without one, the buckets are a guess and the user can't trust them. Output the raw error verbatim, name what you tried, and ask the user whether the issue is on their side (env, migrations, credentials) or whether the pipeline itself needs a hotfix. Do not write the YAML's `applied:` block, do not synthesize a Step 7 report from the partial state, do not guess what the diff "would have" looked like. The right shape is a single `STATUS: BLOCKED (pipeline error)` line plus the error and your next-step ask — nothing else.
...
**Spacing rules** (also see *Formatting and Whitespace* in `docs/agent-guidelines/REVIEW-OUTPUT-SPEC.md`): the glance table stays tight (no blank lines inside the table body); the Notes section is loose (blank line between top-level findings, sub-bullet hugs its parent).
